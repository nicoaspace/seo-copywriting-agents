...
5. **NEVER type or paraphrase SERP URLs from your own memory or from the LLM-generated text summary of `batch_web_search`.** URLs in that text are unreliable. The ONLY trusted source of SERP URLs is the `find_serp_urls` tool, which resolves the real publisher URLs from grounding metadata and HEAD-probes them. Pass the resulting URLs directly into `build_serp_table`. Do not modify, shorten, or "clean up" the URLs returned. **Call `find_serp_urls` EXACTLY ONCE** with the primary keyword + country — never retry with refined queries, never call it twice.
...
### SERP DISCOVERY & SCRAPING — Deterministic two-step

Do NOT extract SERP URLs from the Batch 1 text summaries. Use these two tools in order:

**Step A — Get real SERP URLs (CALL EXACTLY ONCE):**
...
This tool resolves the actual publisher URLs from Gemini's grounding metadata (following Vertex redirect URIs), filters by locale (TLD or path fragment), and HEAD-probes each one. It returns up to 3 verified URLs.

**HARD RULE:** Call `find_serp_urls` **exactly once** per research session, with the primary keyword + country. Whatever URLs come back (even if fewer than 3) are the final SERP set — do **NOT** retry with refined queries, do **NOT** call it a second time. If it returns 2 URLs, Section 1 has 2 rows. If it returns 1, Section 1 has 1 row. If it returns 0, write "No reachable SERP URLs were returned by grounding for this query in this run." and proceed without a table.

**Step B — Scrape all URLs in parallel and build Section 1:**

```
build_serp_table(serp_urls=<the "urls" list returned by find_serp_urls>)
```
